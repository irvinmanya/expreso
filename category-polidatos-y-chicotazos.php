	...
	<div class="container">
		<div class="row">
			...
			<div class="col l3 m12 s12 rowFiltros lineLateralRight">
				...
				<?php if( have_rows('edizqflex-cont',77097) ): ?>
					<?php while ( have_rows('edizqflex-cont',77097) ) : the_row(); ?>
			...
			<div class="col l6 m12 s12">
				<div class="acordBox">
					<ul class="collapsible acordImg"  id="listEdit" data-collapsible="accordion">
						<?php $args = array(
							'posts_per_page' => '9',
							'cat' => 19
						);?>
			...
			<div class="col l3 m12 s12">
				...
				<?php if( have_rows('edderflex-cont',77097) ): ?>
					<?php while ( have_rows('edderflex-cont',77097) ) : the_row(); ?>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#frmEditor').submit(function(e){
			...
			$('#listEdit').html('');
			$.ajax({
				url: 'http://expreso.dhdinc.info/api/get_date_posts/?date='+fecEdit+'&cat=19',
				method: 'post',
				data: {fecEdit: fecEdit},
				dataType: 'json',
				beforeSend: function(){
					$('.acordBox').addClass('acordBoxAct');
				},
				success: function(data){
					$("#plantilla_general").tmpl(data.posts).appendTo("#listEdit");
					$('.acordBox').removeClass('acordBoxAct');
					...
					$.each(data.posts, function(id, item){
						...
						for (var i=0; i<item.attachments.length; i++){
							...
							var contentTxt = item.content;
							insertTimg = attachmentImg + insertTimg;
							insertTXT = contentTxt + insertTXT;
						}

						var id_principal = '#item'+item.id;
						$(id_principal).find('.poliFig').append(insertTimg);
						$(id_principal).find('.campTxt').append(insertTXT);

						$('.acordImg li').find('.collapsible-body').css('display','none');

						$('.acordImg li').eq(0).addClass('active');
						$('.acordImg li').eq(0).find('.collapsible-header').addClass('active');
						$('.acordImg li').eq(0).find('.collapsible-body').css('display','block');
